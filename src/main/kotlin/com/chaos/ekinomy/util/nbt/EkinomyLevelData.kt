...
class EkinomyLevelData(name: String? = null) : WorldSavedData(name ?: Ekinomy.MODID), Supplier<EkinomyLevelData> {
    var dataCollection: MutableList<PlayerBalanceData> = mutableListOf()
    ...
    override fun read(nbt: CompoundNBT) {
        ...
        list.forEach {
            dataCollection.add(NBTHelper.fromCompoundNBT(it as CompoundNBT))
        }
    }

    override fun write(compound: CompoundNBT): CompoundNBT {
        val list = ListNBT()
        dataCollection.forEach { list.add(NBTHelper.toCompoundNBT(it)) }
